...
void mamba_block_init(struct mamba_block* block,
                      ...
                      double* pState, int d_model, int d_state) {
  block->A = pA;
  block->B = pB;
  block->C = pC;
  block->h = pState;
...
void mamba_block_execute(struct mamba_block* block,
  ...
  // Temporary buffer for (B @ x) - allocated on HEAP (Key L2 SRAM)
  double* u = (double*)malloc(d_state * sizeof(double));

  // ---
  // STEP 1: Project Input (u = B @ x)
  // Matrix-Vector Multiply: (d_state, d_model) @ (d_model) -> (d_state)
  // ---
  for (size_t i = 0; i < d_state; i++) {
    size_t vl_gemv;
    // Initialize accumulator vector for the dot product
    vfloat64m1_t acc_vec = __riscv_vfmv_v_f_f64m1(0.0, 1);

    // Strip-mined loop over the columns of B and elements of x
    for (size_t j = 0; j < d_model; j += vl_gemv) {
      ...
      // Reduce and accumulate the products
  ...
  // Element-wise operation: (d_state) + (d_state)
  // ---
  for (size_t l = 0; l < d_state; l += vl) {
    ...
    vfloat64m8_t u_vec = agni_vload_e64m8(u + l, vl); // Load from temp buffer
  ...
  // Matrix-Vector Multiply: (d_model, d_state) @ (d_state) -> (d_model)
  // ---
  for (size_t i = 0; i < d_model; i++) {
    ...
    // Strip-mined loop over the columns of C and elements of h
